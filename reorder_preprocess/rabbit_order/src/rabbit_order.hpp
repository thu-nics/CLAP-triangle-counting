...
graph aggregate(std::vector<std::vector<edge> > adj) {
  ...
  // Sorts the vertices in degrees from small to large
  ...
  #pragma omp parallel reduction(+: npend) reduction(max: tmax) reduction(+: ttotal)
  {
    ...
    const int        tid    = omp_get_thread_num(); //CPU thread idx
    std::deque<vint> tops, pends; // deque is a bi-ended queue container; top contains the root vertices(Modularity Q <=0)
...
std::unique_ptr<vint[]> compute_perm(const graph& g) {
  ...
  const vint        ncom = static_cast<vint>(g.tops->size());  //community总数
