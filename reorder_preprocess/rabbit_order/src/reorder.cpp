...
std::shared_ptr<int> reorder(adjacency_list adj) {
...
#pragma omp parallel for
  ...
  // std::map<int, int> perm_mapping1;
  // for (int i = 0; i < g.n(); i++)
  //   perm_mapping1.insert(std::pair<int, int>(i, p[i]));

  // revised by ys
  // auto perm_mapping = (int *)malloc(2 * g.n() * sizeof(int));
  std::shared_ptr<int> perm_mapping(new int[2 * g.n()],
                                    [](int *p) { delete[] p; });
  // memset(perm_mapping, 0, sizeof(perm_mapping));

  for (unsigned int i = 0; i < g.n(); i++) {
    perm_mapping.get()[0 + i] = p[i];
    perm_mapping.get()[g.n() + i] = c[i];
  }
...
torch::Tensor rabbit_reorder(torch::Tensor in_edge_index) {
  int src, dst;
  ...
  torch::Tensor out_edge_index =
      torch::zeros({6, in_edge_index.size(1)}, options);
  ...
  for (int i = 0; i < numedges; i++) {
    ...
    out_edge_index_ptr[0][i] = mapping.get()[src];
    out_edge_index_ptr[1][i] = mapping.get()[dst];
    out_edge_index_ptr[2][i] = mapping.get()[src + n_vertices];
    out_edge_index_ptr[3][i] = mapping.get()[dst + n_vertices];
    out_edge_index_ptr[4][i] = src;
    out_edge_index_ptr[5][i] = dst;
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("reorder", &rabbit_reorder,
        "Get the reordered node id mapping: old_id --> new_id");
}
