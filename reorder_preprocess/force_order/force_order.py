...
class degree_difference_force:
    ...
    def __call__(self, index, degree) -> Any:
        force = 2*self.alpha*(len(self.degree_list)-1)*((degree-self.degree_list[index]))/((self.degree_list[-1]-self.degree_list[0])**1.5)
        return force
...
def main(args, graph_name):
    ...
    for i in range(layer):
        comm_mapping_list[i], comm_list_list[i] = community_preprocessing(new_comm_dict_list[i])
    ...
    adj = {u: [v for v in graph[u] if v != u] for u in graph}
    ...
    if args.output_format == 'edge_list':
        ...
        with open(args.output_file, 'wb') as f:
            ...
            write_list = [num_node]+[len(pos_list)]+pos_list+[len(neigh_list)]+neigh_list
...
if __name__ == '__main__':
    ...
    def set_args(args, format, order, graph_name):
        ...
        if format == 'edge_list':
            args.output_file = '../../data/edge_list/'+order+'/'+graph_name+'.txt'
