...
class self_defined_force_d:
    ...
    def __call__(self, index, degree) -> Any:
        ...
        force = self.alpha*(degree-self.avg_degree) # define the force by yourself, e.g., self.alpha*(degree-self.avg_degree)
        return force
    ...
    def __call__(self, index, comm_id_list: List, comm_list_list: List = None, comm_center_list: List = None, node_mapping: Dict = None) -> Any:
        ...
        for i in range(self.layer): 
            if comm_center_list[i][comm_id_list[i]] != -1:
                ...
                cluster_center = np.sum(new_cluster)/len(new_cluster) # define the center by yourself, e.g., np.sum(new_cluster)/len(new_cluster)
                comm_center_list[i][comm_id_list[i]] = cluster_center
            force += self.alpha[i]*(cluster_center-index) # define the force by yourself, e.g., self.alpha[i]*(cluster_center-index)
        return force