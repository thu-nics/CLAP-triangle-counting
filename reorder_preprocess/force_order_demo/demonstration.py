    ...
    def __call__(self, index, comm_id_list: List, comm_list_list: List = None, comm_center_list: List = None, node_mapping: Dict = None) -> Any:
        ...
        for i in range(self.layer): 
            if comm_center_list[i][comm_id_list[i]] != -1:
                ...
                new_cluster = [node_mapping[node] for node in comm_list_list[i][comm_id_list[i]]]
...
class degree_difference_force:
    ...
    def __call__(self, index, degree) -> Any:
        force = 2*self.alpha*(len(self.degree_list)-1)*((self.degree_list[index]-degree))/((self.degree_list[-1]-self.degree_list[0])**1.5)
        return force
...
class degree_difference_force_linear:
    ...
    def __init__(self, alpha = 1.0, degree_list: List = None) -> None:
        ...
        self.degree_list = degree_list
    def __call__(self, index, degree) -> Any:
        force = self.alpha*(len(self.degree_list)-1)*(degree-self.degree_list[index])/(self.degree_list[-1]-self.degree_list[0])
        return force
...
def main(args, graph_name, order):
    ...
    for i in range(layer):
        comm_mapping_list[i], comm_list_list[i] = community_preprocessing(new_comm_dict_list[i], avg_degree)
    ...
    if control_type == 'diff':
        f_control = degree_difference_force(alpha = alpha_control, degree_list = degree_list)
        ...
        f_control = degree_difference_force_linear(alpha = alpha_control, degree_list = degree_list)
    ...
    node_mapping = {node:node for node in graph.nodes}
    relabel_list = [[node, node, node, graph.degree(node)] for node in graph.nodes] # 0:init id 1:relabled id 2:float num 3:community of this node 4:degree
    for node_tuple in relabel_list:
        comm_id_list = [comm_mapping_list[i][new_comm_dict_list[i][node_tuple[0]]] for i in range(layer)]
    ...
    if mode == 'simutaneous':
        while(iterate_count<iterate_time):
            comm_center_list = [[-1 for i in range(len(comm_list_list[j]))] for j in range(layer)]
            for node_tuple in relabel_list:
    ...
    graph = nx.relabel_nodes(graph, node_mapping, copy = True)
    anim = animation.FuncAnimation(fig, dma, frames=len(dma), blit=True, repeat_delay=1000, init_func=dma.init_graph)
    # get current time
    now = time.localtime() 
    current_time = time.strftime("%H_%M_%S", now)
    anim.save('dynamic_graph_'+str(current_time)+'.gif', writer='imagemagick', fps=1)
    plt.show()
    plt.close()
    ...
    if args.output_format == 'edge_list':
        ...
        WriteEdgeList(args.output_file ,edge_list, first_line= '#'+str(num_node)+' '+str(num_edge))
    else:
        raise NotImplementedError
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_file', type=str,
        help='name of the input edge_list file'
    )
    parser.add_argument('--output_file', type=str,
        help='name of the output dict file'
    )
